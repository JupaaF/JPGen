"""Place particles through progressive temporary-radius growth and relaxation."""
...
class ProgressiveGrowth(PlacementStrategy):
    ...
    def from_config(cls, config):
        mapping(config, "placement", {"method", "initial_scale", "initial_increment", "min_increment",
        ...
        for name, default in (("initial_scale", 0.25), ("initial_increment", 0.05),
            ...
            values[name] = number(config.get(name, default), f"placement.{name}", 0, 1, strict_min=True)
        if values["initial_scale"] >= 1:
            raise ConfigurationError("placement.initial_scale must be smaller than 1.")
        ...
        return cls(
            ...
            max_stages=integer(config.get("max_stages", 200), "placement.max_stages"),
            **values,
        )
    ...
    def place(self, request: PlacementRequest, observer=None):
        ...
        if not result.converged:
            raise PackingGenerationError(f"Initial growth stage at scale {scale:.6g} did not converge; maximum overlap excess {result.max_excess:.6g}.")
        ...
        while scale < 1:
            if len(history) >= self.max_stages:
                raise PackingGenerationError(f"Growth stage limit reached at scale {scale:.6g}; final radii were not reached.")
            trial_scale = min(1.0, scale + increment)
            if trial_scale <= scale:
                raise PackingGenerationError("Growth increment is too small to change the radius scale.")
            ...
            if result.converged:
                ...
                if increment < self.min_increment:
                    raise PackingGenerationError(f"Growth stalled at scale {scale:.6g}; increment fell below min_increment. Final radii were not reached.")
