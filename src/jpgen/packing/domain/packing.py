"""Particle packing aggregate and reproducibility metadata."""
...
class ParticlePacking:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.metadata, PackingMetadata):
            raise ValueError("Packing metadata must be PackingMetadata.")
        self.validate()

    def with_metadata(self, metadata):
        if not isinstance(metadata, PackingMetadata):
            raise ValueError("Packing metadata must be PackingMetadata.")
        result = object.__new__(ParticlePacking)
