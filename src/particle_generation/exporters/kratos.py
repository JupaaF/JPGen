"""Kratos DEM particle model part. Solver settings and walls are external."""


def write_mdpa(path, particles):
    with open(path, "w", encoding="utf-8") as file:
        ...
        for pid, position in zip(particles.ids, particles.positions):
        ...
        values = {"RADIUS": particles.radii}
        for axis, letter in enumerate("XYZ"):
            values[f"VELOCITY_{letter}"] = particles.velocities[:, axis]
            values[f"ANGULAR_VELOCITY_{letter}"] = particles.angular_velocities[:, axis]
        for name, array in values.items():
            file.write(f"\nBegin NodalData {name}\n")
            for pid, value in zip(particles.ids, array):
